...
describe("If", () => {
    describe("Assert", () => {
        ...
        function test2(b, c) {
            if (typeof b !== "number")
                throw new Error("Expected b to be number.");
            if (b !== 5)
                throw new Error("Expected b to satisfy `$self === 5`.");
            if (!(c instanceof Array))
                throw new Error("Expected c to be string[].");
            for (let i_1 = 0; i_1 < c.length; i_1++) {
                const x_1 = c[i_1];
                if (typeof x_1 !== "string")
                    throw new Error("Expected " + ("c[" + i_1 + "]") + " to be string.");
            }
            if (c.length < 10)
        ...
        it("Throw when the type doesn't match", () => {
            (0, chai_1.expect)((0, utils_1.call)(test2, "Some str")).to.throw("Expected b to be number.");
            (0, chai_1.expect)((0, utils_1.call)(test2, 5, ["a", "b", "c", 33])).to.throw("Expected c[3] to be string.");
